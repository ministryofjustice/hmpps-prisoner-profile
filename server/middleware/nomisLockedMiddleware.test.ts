...
describe('nomisLockedMiddleware', () => {
  it('should flash isLocked and redirect when NomisLockedError happens', () => {
    const req = {
      flash: jest.fn(),
      body: { foo: 'bar' },
    ...
    const err = new NomisLockedError('Resource Locked')
    const next = jest.fn()

    nomisLockedMiddleware(err, req, res, next)
    ...
    expect(res.redirect).toHaveBeenCalledWith('/something')
