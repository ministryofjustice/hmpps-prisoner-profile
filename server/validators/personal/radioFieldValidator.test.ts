...
describe('Text field length validator', () => {
  ...
  it.each([
    [{ fieldName: 'fieldValue', fieldDisplayName: 'Field value' }, 'Select Field value.'],
    [{ fieldName: 'fieldTwo', fieldDisplayName: 'Field two' }, 'Select Field two.'],
  ])(
    'Validations: %s: %s',
    async ({ fieldName, fieldDisplayName }: { fieldName: string; fieldDisplayName: string }, errorMessage: string) => {
      const body = {}
      const validator = radioFieldValidator({
        fieldName,
        fieldDisplayName,
