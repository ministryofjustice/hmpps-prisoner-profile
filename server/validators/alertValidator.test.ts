...
describe('Validation middleware - Add Alert', () => {
  it('should pass validation with good data', async () => {
    const alertForm = {
      ...
      activeFrom: formatDate(new Date().toISOString(), 'short'),
      activeTo: formatDate(addDays(new Date(), 10).toISOString(), 'short'),
    }
  ...
  it('should fail validation with no data', async () => {
    ...
    expect(result).toEqual([
      ...
      { text: 'Enter why you are creating this alert', href: '#description' },
      { text: 'Select the alert start date', href: '#activeFrom' },
    ])
  })
  ...
  it('should fail validation with bad data', async () => {
    ...
    expect(result).toEqual([
      { text: 'Enter why you are creating this alert using 4,000 characters or less', href: '#description' },
      { text: 'Enter a real date in the format DD/MM/YYYY - for example, 27/03/2023', href: '#activeFrom' },
    ])
  })
  ...
  it.each([
    ['on', '01/01/2000'],
    ['before', '01/01/1999'],
  ])('should fail validation with expiry date %s the start date', async (_, activeTo) => {
    const alertForm = {
      alertType: 'X',
      alertCode: 'X',
      description: 'X',
...
describe('Validation middleware - Add more details', () => {
  ...
  it('should fail validation with no data', async () => {
    ...
    expect(result).toEqual([{ text: 'Enter your comments on this alert', href: '#description' }])
  })

  it('should fail validation with text over 4000 characters', async () => {
    const alertForm = {
      description:
    ...
    expect(result).toEqual([{ text: 'Enter your comments using 4,000 characters or less', href: '#description' }])
  })
})
...
describe('Validation middleware - Close', () => {
  ...
  it('should pass validation with good data - not today', async () => {
    const alertForm = {
      description: 'Note text',
      activeTo: formatDate(formatDateISO(addDays(new Date(), 1)), 'short'),
      today: 'no',
    }
  ...
  it('should fail validation with no data', async () => {
    ...
    expect(result).toEqual([
      { text: 'Enter your comments on this alert', href: '#description' },
      { text: 'Enter an end date in the format DD/MM/YYYY - for example, 27/03/2023', href: '#activeTo' },
    ])
  })
  ...
  it('should fail validation with today = no and date not after today', async () => {
    const alertForm = {
      ...
      activeTo: formatDate(formatDateISO(new Date()), 'short'),
      today: 'no',
    }
