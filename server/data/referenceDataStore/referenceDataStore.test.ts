...
import { CountryReferenceDataCodes } from '../localMockData/personIntegrationReferenceDataMock'
...
describe('prisonRegisterStore', () => {
  ...
  it('should set reference data codes', async () => {
    ...
    // When
    await referenceDataStore.setReferenceData('COUNTRY', CountryReferenceDataCodes, durationHours)

    // Then
    expect(redisClient.set).toHaveBeenCalledWith(
      'reference_data_COUNTRY',
      JSON.stringify(CountryReferenceDataCodes),
  ...
  it('should get reference data given redis client returns reference data', async () => {
    // Given
    const serializedData = JSON.stringify(CountryReferenceDataCodes)
    ...
    // Then
    expect(returnedData).toStrictEqual(CountryReferenceDataCodes)
