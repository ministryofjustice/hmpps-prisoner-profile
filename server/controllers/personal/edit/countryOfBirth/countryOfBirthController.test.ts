    ...
    describe('edit', () => {
      ...
      it('Renders the default edit page with the correct data from the prison person API', async () => {
        ...
        expect(res.render).toHaveBeenCalledWith('pages/edit/radioFieldWithAutocomplete', {
          ...
          errors: [],
          radioOptions: expect.arrayContaining([
            expect.objectContaining({ text: 'England', value: 'ENG', checked: true }),
          ]),
          autocompleteOptions: expect.arrayContaining([expect.objectContaining({ text: 'France', value: 'FRA' })]),
      ...
      it('Populates the radio buttons from the flash', async () => {
        ...
        expect(res.render).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            autocompleteSelected: false,
            radioOptions: expect.arrayContaining([expect.objectContaining({ checked: true, value: 'ENG' })]),
          }),
        )
      })
      ...
      it('Populates the autocomplete from the flash', async () => {
        ...
        expect(res.render).toHaveBeenCalledWith(
          expect.anything(),
          expect.objectContaining({
            autocompleteOptions: expect.arrayContaining([expect.objectContaining({ selected: true, value: 'FRA' })]),
          }),
        )
      })
