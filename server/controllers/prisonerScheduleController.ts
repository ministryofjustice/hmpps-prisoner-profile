...
export default class PrisonerScheduleController {
  ...
  public async displayPrisonerSchedule(req: Request, res: Response, prisonerData: Prisoner)  {
    ...
    const startOfWeek = when === 'nextWeek' ? oneWeekToday : todaysDate
    ...
    const filterMorning = (activities: GetEventScheduleItem[]) =>
      activities && activities.filter((activity) => getHours(parseInt(activity.startTime)) < 12)

    const filterAfternoon = (activities: GetEventScheduleItem[]) =>
      activities &&
      activities.filter(
        (activity) => getHours(parseInt(activity.startTime)) < 17)

    const filterEveningDuties = (activities: GetEventScheduleItem[]) =>
        activities && activities.filter((activity) =>  getHours(parseInt(activity.startTime)) >= 17)

    const byStartTimeThenByEndTime = (a: GetEventScheduleItem, b: GetEventScheduleItem) => {
      isBefore(parseInt(a.startTime), parseInt(b.startTime))
      if (isBefore(parseInt(a.startTime), parseInt(b.startTime))) return -1
    
      if (isAfter(parseInt(a.startTime), parseInt(b.startTime))) return 1
    
      if (!a.endTime) return -1
      if (!b.endTime) return 1
    
      if (isBefore(parseInt(a.endTime), parseInt(b.endTime))) return -1
      if (isAfter(parseInt(a.endTime), parseInt(b.endTime))) return 1
