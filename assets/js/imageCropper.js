...
;(function (Modules) {
  function ImageCropper($imageCropper) {
    ...
    this.$targetWidth = 427
    this.$targetHeight = 570
    this.$fileName = this.$image.dataset.fileName
    this.$fileType = this.$image.dataset.fileType
  }
  ...
  ImageCropper.prototype.updateAriaLabel = function () {
    ...
    const portionSelected = (cropBoxData.height * cropBoxData.width) / (imageData.height * imageData.width)
    ...
    this.$imageCropper.ariaLabel =
      'Image to be cropped. ' + percentage + '% of the image, centered on the ' + positionText + 'is selected.'
  }
...
var ImageCropper = window.GOVUK.Modules.ImageCropper
var $imageCropper = document.querySelector('[data-module="image-cropper"]')
if ($imageCropper) {
