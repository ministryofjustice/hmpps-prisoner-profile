...
;(function (Modules) {
  ...
  function MultiFileUpload(params) {
    ...
    this.container.addClass('dps-multi-file-upload--enhanced')

    this.feedbackContainer = this.container.find('.dps-multi-file__uploaded-files')
    this.setupFileInput()
    this.setupDropzone()
    this.setupLabel()
    this.setupStatusBox()
    this.container.on('click', '.dps-multi-file-upload__delete', $.proxy(this, 'onFileDeleteClick'))
  }

  MultiFileUpload.prototype.setupDropzone = function () {
    this.fileInput.wrap('<div class="dps-multi-file-upload__dropzone" />')
    this.dropzone = this.container.find('.dps-multi-file-upload__dropzone')
  ...
  MultiFileUpload.prototype.setupFileInput = function () {
    this.fileInput = this.container.find('.dps-multi-file-upload__input')
    this.fileInput.addClass('dps-hidden')
  ...
  MultiFileUpload.prototype.onDragOver = function (e) {
    e.preventDefault()
    this.dropzone.addClass('dps-multi-file-upload--dragover')
  }

  MultiFileUpload.prototype.onDragLeave = function () {
    this.dropzone.removeClass('dps-multi-file-upload--dragover')
  }
  ...
  MultiFileUpload.prototype.onFileFocus = function (e) {
    this.label.addClass('dps-multi-file-upload--focused')
  }

  MultiFileUpload.prototype.onFileBlur = function (e) {
    this.label.removeClass('dps-multi-file-upload--focused')
  }

  MultiFileUpload.prototype.getSuccessHtml = function (success) {
    return `<span class="dps-multi-file-upload__success"> <svg class="moj-banner__icon" fill="currentColor" role="presentation" focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" height="25" width="25"><path d="M25,6.2L8.7,23.2L0,14.1l4-4.2l4.7,4.9L21,2L25,6.2z"/></svg>${success.messageHtml}</span>`
  }

  MultiFileUpload.prototype.getErrorHtml = function (error) {
    return `<span class="dps-multi-file-upload__error"> <svg class="moj-banner__icon" fill="currentColor" role="presentation" focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" height="25" width="25"><path d="M13.6,15.4h-2.3v-4.5h2.3V15.4z M13.6,19.8h-2.3v-2.2h2.3V19.8z M0,23.2h25L12.5,2L0,23.2z"/></svg>${error.message}</span>`
  }
  ...
  MultiFileUpload.prototype.getDeleteButtonHtml = function (file) {
    return `<button class="dps-multi-file-upload__delete govuk-button govuk-button--secondary govuk-!-margin-bottom-0" type="button" name="delete" value="${file.filename}">
  ...
  MultiFileUpload.prototype.uploadFile = function (file) {
    ...
    $.ajax({
      ...
      success: $.proxy(function (response) {
        if (response.error) {
          item.find('.dps-multi-file-upload__message').html(this.getErrorHtml(response.error))
          this.status.html(response.error.message)
        } else {
          item.find('.dps-multi-file-upload__message').html(this.getSuccessHtml(response.success))
          this.status.html(response.success.messageText)
        }
        item.find('.dps-multi-file-upload__actions').append(this.getDeleteButtonHtml(response.file))
    ...
    $.ajax({
      ...
      success: $.proxy(function (response) {
        if (response.error) {
          ...
          button.parents('.dps-multi-file-upload__row').remove()
          if (this.feedbackContainer.find('.dps-multi-file-upload__row').length === 0) {
  ...
  Modules.MultiFileUpload = MultiFileUpload
})(window.GOVUK.Modules)

var MultiFileUpload = window.GOVUK.Modules.MultiFileUpload
