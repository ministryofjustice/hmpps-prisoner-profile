...
When we do add client side scripts, we add any reusable or non-page specific
typescript or javascript in our `shared` folder, and import it in `shared/index.ts`. This will get bundled
all together and minified by esbuild. All pages will load and run this shared
javascript. Any page specific typescript/javascript files outside the `shared` folder are kept
